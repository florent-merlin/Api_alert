...
module.exports.createAlert = async (req, res) => {
	...
	if (user) {
		...
		res.status(401).json({
			message: 'veuillez vous connecter afin de pouvoir créer une alert',
		});
	}
};
...
module.exports.updateAlert = async (req, res) => {
	...
	try {
		...
		res.json({ msg: `modificatiion de l'alerte réussie! info_idAlert: ${req.params.id}` });
	} catch (err) {
